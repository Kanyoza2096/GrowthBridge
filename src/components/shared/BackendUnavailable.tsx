...
export function BackendUnavailable({
  ...
  const status = { provider: 'supabase', supabaseUrl: publicConfig.NEXT_PUBLIC_SUPABASE_URL, allowMock: false };
  ...
  const devHint = (
    <div className="rounded-xl p-3 text-xs border border-dashed border-[var(--border-strong)] bg-[var(--chip-orange-bg)] text-[var(--text-secondary)]">
      ...
      <p className="mt-2 opacity-80">
        Mock mode is intended for local development only; production must use the live Supabase backend.
      </p>
    </div>
  );

  const checks = [
    { label: 'API URL configured', ok: Boolean(status.supabaseUrl) },
    {
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl p-3.5 text-xs border border-[var(--border-subtle)] bg-[var(--surface-page)] text-[var(--text-tertiary)]">
                ...
                <dl className="grid grid-cols-[auto_1fr] gap-y-1.5 gap-x-3">
                  ...
                  <dd className="font-mono text-[11px] truncate text-[var(--text-primary)]" title={status.supabaseUrl}>
                    {status.supabaseUrl}
                  </dd>
