...
interface CardProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  variant?: 'glass' | 'solid' | 'gradient' | 'admin';
  hoverEffect?: boolean;
}
...
export function Card({
  ...
  const variants: Record<NonNullable<CardProps['variant']>, string> = {
    ...
    gradient:
      'bg-gradient-brand border border-white/10 text-white',
    admin:
      'bg-[var(--admin-surface-card)] border border-[var(--admin-border)] text-[var(--admin-text-primary)] shadow-[var(--admin-shadow-card)]',
  };
