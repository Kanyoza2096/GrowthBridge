...
export function WhyGrowthbridge() {
  ...
  return (
    <section className="py-16 sm:py-20 bg-[var(--surface-soft)] relative">
      <Container size="lg">
        <SectionHeading
          badge="Core Values"
          badgeVariant="orange"
            ...
            <ScrollReveal key={val.title} delay={idx * 100}>
              <Card variant="solid" className="h-full group hover:border-[var(--border-accent)]">
                <div className="flex flex-col sm:flex-row items-start gap-4 sm:space-x-5">
                  <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-[var(--surface-subtle)] border border-[var(--border-subtle)] flex items-center justify-center text-xl sm:text-2xl group-hover:scale-110 transition-transform flex-shrink-0">
                    {val.icon}
                  </div>
