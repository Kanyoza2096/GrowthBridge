...
export function HowWeCreateImpact() {
  ...
  return (
    <section className="py-16 sm:py-20 relative bg-[var(--surface-page)]">
      <Container size="lg">
        <SectionHeading
          badge="Impact Engine"
          badgeVariant="purple"
          title="How We Create Lasting Impact"
          subtitle="Our 4-step impact lifecycle bridges skill gaps and creates economic opportunities across South Africa."
        />
          ...
          {steps.map((item, idx) => (
            <ScrollReveal key={item.step} delay={idx * 150}>
              <div className="relative p-5 sm:p-6 rounded-2xl bg-[var(--card-surface)] border border-[var(--card-border)] shadow-[var(--card-shadow)] space-y-4 h-full flex flex-col justify-between group hover:border-[var(--border-accent)] transition-colors">
                ...
                <div className="w-full h-1 bg-[var(--gradient-brand)] rounded-full opacity-40 group-hover:opacity-100 transition-opacity mt-4" />
              </div>
            </ScrollReveal>
          ))}
        </div>
