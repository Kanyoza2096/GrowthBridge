  ...
  return (
    <div className="public-page space-y-10 sm:space-y-16 pb-16 sm:pb-20">
      ...
      <section className="public-page-hero py-12 sm:py-16 md:py-20">
        <Container size="lg">
          <div className="max-w-3xl mx-auto text-center space-y-4">
            ...
            {/* Mode Switcher */}
            <div className="flex flex-col sm:flex-row justify-center gap-2 sm:gap-3 pt-4 w-full max-w-md mx-auto sm:max-w-none">
              <button
                onClick={() => setActiveTab('hire')}
                className={`min-h-11 px-5 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer w-full sm:w-auto ${
                  activeTab === 'hire'
        ...
        <section>
          <Container size="lg">
            ...
            {isLoading ? (
              ...
              <PublicEmptyState
                title="No matching talent profiles"
                description="Try another search or department, or apply to join the Growthbridge talent pipeline."
              />
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
                {filteredMembers.map((member, idx) => (
                  <ScrollReveal key={member.id} delay={idx * 100}>
                    <Card
                      variant="solid"
                      className="h-full flex flex-col justify-between p-4 sm:p-6 space-y-4"
                    >
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center space-x-4">
                          <div className="w-12 h-12 rounded-2xl bg-gradient-brand flex items-center justify-center font-bold text-white text-lg">
      ...
      {activeTab === 'apply' && (
        <section>
          <Container size="sm">
            <div className="max-w-2xl mx-auto w-full"><TalentApplicationForm /></div>
          </Container>
        </section>
      )}
    </div>
