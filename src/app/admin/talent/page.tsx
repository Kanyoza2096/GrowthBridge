    ...
    return (
      <div className="space-y-6">
        ...
        <Card variant="glass" className="p-8 text-center">
          <p className="text-slate-400">You do not have permission to view the Talent Hub.</p>
        </Card>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredTalent.map((t) => {
          ...
          return (
            <Card key={t.id} variant="glass" className="p-6 space-y-4 flex flex-col">
              ...
              <div className="flex flex-wrap gap-1.5">
                {t.skills.slice(0, 4).map((sk, i) => (
                  <span
                    key={i}
                    className="px-2 py-0.5 text-[10px] font-semibold rounded-md bg-slate-900/80 border border-slate-800 text-emerald-300"
                  >
                    {sk}
                  </span>
                ))}
                {t.skills.length > 4 && (
                  <span className="px-2 py-0.5 text-[10px] font-semibold rounded-md bg-slate-900/80 border border-slate-800 text-slate-400">
                    +{t.skills.length - 4}
                  </span>
                )}
              </div>
        ...
        {filteredTalent.length === 0 && (
          <Card variant="glass" className="p-8 col-span-full text-center">
            <p className="text-slate-400">No talent profiles match the current filters.</p>
          </Card>
        )}
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="w-full space-y-1.5">
              ...
              <select
                ...
                className="w-full rounded-xl bg-slate-900/60 border border-slate-800 text-slate-100 px-4 py-3 text-sm focus:outline-none focus:border-[#16A36A] focus:ring-1 focus:ring-[#16A36A] cursor-pointer"
              >
