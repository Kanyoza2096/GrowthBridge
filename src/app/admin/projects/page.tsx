  ...
  return (
    <div className="admin-page space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {filtered.map((proj) => (
          <Card key={proj.id} variant="admin" className="p-5 space-y-4 flex flex-col">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-[var(--admin-text-secondary)] mt-0.5">
                  Client: <span className="text-[var(--admin-text-primary)]">{proj.client || '—'}</span>
                </p>
              </div>
              <button
                onClick={() => toggleFeatured(proj)}
                title={proj.featured ? 'Unfeature' : 'Feature'}
                className={`text-2xl leading-none transition-colors cursor-pointer ${proj.featured ? 'text-amber-400' : 'text-[var(--admin-text-tertiary)] hover:text-amber-400/70'}`}
                disabled={!canUpdate}
              >
                ★
              </button>
            ...
            <p className="text-sm text-[var(--admin-text-secondary)] line-clamp-2">{proj.description || <span className="text-[var(--admin-text-tertiary)] italic">No description</span>}</p>

            {proj.technologies.length > 0 && (
              <div className="space-y-1.5">
                <div className="text-[10px] uppercase tracking-wider text-[var(--admin-text-tertiary)] font-semibold">Tech Stack</div>
                <div className="flex flex-wrap gap-1.5">
                  {proj.technologies.slice(0, 6).map((t) => (
                    <span key={t} className="px-2 py-0.5 rounded-lg bg-[var(--gb-navy-800)]/60 border border-[var(--gb-navy-800)]/50 text-[11px] text-blue-200">
                      {t}
                    </span>
                  ))}
                  {proj.technologies.length > 6 && (
                    <span className="px-2 py-0.5 text-[11px] text-[var(--admin-text-tertiary)]">+{proj.technologies.length - 6}</span>
                  )}
                </div>
              </div>
            )}
            ...
            {proj.impactStats.length > 0 && (
              <div className="pt-3 border-t border-[var(--admin-border)] space-y-1.5">
                <div className="text-[10px] uppercase tracking-wider text-[var(--admin-text-tertiary)] font-semibold">Impact</div>
                <div className="grid grid-cols-2 gap-2">
                  {proj.impactStats.slice(0, 4).map((s, i) => (
                    <div key={`${proj.id}-${s.label}-${i}`} className="bg-[var(--admin-surface-card)]/50 rounded-xl px-3 py-2 border border-[var(--admin-border)]">
                      <div className="text-[10px] uppercase tracking-wider text-[var(--admin-text-tertiary)] font-semibold truncate">{s.label}</div>
                      <div className="text-sm font-bold text-[var(--gb-green-400)] truncate">{s.value}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {filtered.length === 0 && (
          <Card variant="admin" className="md:col-span-2 p-10 text-center">
            <p className="text-[var(--admin-text-secondary)]">No projects match your filters.</p>
          </Card>
        )}
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="flex items-end">
              <label className="inline-flex items-center gap-3 text-sm text-[var(--admin-text-primary)] cursor-pointer select-none bg-[var(--admin-surface-card)]/60 border border-[var(--admin-border)] rounded-xl px-4 py-3 w-full">
                <input
                  type="checkbox"
                  checked={form.featured}
                  onChange={(e) => setForm({ ...form, featured: e.target.checked })}
                  className="w-4 h-4 rounded accent-amber-500 bg-[var(--admin-surface-soft)] border-[var(--admin-border-strong)]"
                />
                <div>
                  <div className="font-semibold">Featured Project</div>
                  <div className="text-[11px] text-[var(--admin-text-secondary)]">Show in homepage showcase</div>
                </div>
              </label>
            </div>
          </div>
