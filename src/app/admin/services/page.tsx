  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filtered.map((svc) => (
          <Card key={svc.id} variant="glass" className="p-5 space-y-4 flex flex-col">
            ...
            <p className="text-sm text-slate-300 line-clamp-2">{svc.description || <span className="text-slate-500 italic">No description</span>}</p>

            <div className="space-y-2">
              <div className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">Features</div>
              <ul className="space-y-1">
                ...
                {svc.features.length > 4 && (
                  <li className="text-xs text-slate-500">+{svc.features.length - 4} more</li>
                )}
                {svc.features.length === 0 && <li className="text-xs text-slate-500 italic">No features listed</li>}
              </ul>
            </div>

            <div className="pt-3 border-t border-slate-800 space-y-2 mt-auto">
              <div className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">SEO</div>
              <div className="text-xs text-slate-300 space-y-0.5">
                <div>Title: <span className="text-white">{svc.seoTitle || <span className="text-slate-500">—</span>}</span></div>
                <div className="line-clamp-1">Desc: <span className="text-slate-400">{svc.seoDescription || '—'}</span></div>
                {svc.seoKeywords.length > 0 && (
                  <div className="flex flex-wrap gap-1 pt-1">
                    {svc.seoKeywords.slice(0, 4).map((k) => (
                      <span key={k} className="px-1.5 py-0.5 rounded bg-[#123B5D]/40 text-[10px] text-blue-300">{k}</span>
                    ))}
                    {svc.seoKeywords.length > 4 && <span className="text-[10px] text-slate-500">+{svc.seoKeywords.length - 4}</span>}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              <label className="inline-flex items-center gap-2 text-xs text-slate-300 cursor-pointer select-none">
                <input
                  ...
                  className="w-4 h-4 rounded accent-[#16A36A] bg-slate-800 border-slate-700"
                />
                Publish
              </label>
        ...
        {filtered.length === 0 && (
          <Card variant="glass" className="md:col-span-2 lg:col-span-3 p-10 text-center">
