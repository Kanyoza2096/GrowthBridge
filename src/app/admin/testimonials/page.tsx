  ...
  return (
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        ...
        >
          <svg
            className={cn(sizeCls, star <= displayValue ? 'text-amber-400 fill-amber-400' : 'text-[var(--admin-text-tertiary)] fill-slate-700')}
            viewBox="0 0 20 20"
            stroke="currentColor"
            strokeWidth="1"
          >
...
function Avatar({ name }: { name: string }) {
  ...
  return (
    <div
      className="w-11 h-11 rounded-full flex items-center justify-center text-white font-bold text-sm shrink-0 border-2 border-[var(--admin-border)]"
    ...
    <div className="admin-page space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filtered.map((t) => {
          ...
          return (
            <Card key={t.id} variant="admin" className="p-6 space-y-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  <Avatar name={t.authorName} />
                  <div>
                    <h3 className="text-sm font-bold text-white leading-tight">{t.authorName}</h3>
                    <p className="text-xs text-[var(--admin-text-secondary)]">{t.authorTitle}</p>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      t.featured
                        ...
                        : 'bg-[var(--admin-surface-card)] text-[var(--admin-text-secondary)] border-[var(--admin-border)] hover:border-[var(--admin-border-strong)]'
                    )}
                  >
              ...
              <blockquote className="text-sm text-[var(--admin-text-secondary)] leading-relaxed bg-[var(--admin-surface-card)]/60 p-4 rounded-xl border border-[var(--admin-border)] relative">
                <span className="absolute -top-2 left-3 text-[var(--gb-green-600)] text-2xl font-serif leading-none select-none">&ldquo;</span>
                <span className="line-clamp-3">{t.content}</span>
                <span className="absolute -bottom-3 right-3 text-[var(--gb-green-600)] text-2xl font-serif leading-none select-none">&rdquo;</span>
              </blockquote>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-[var(--admin-border)]">
                {project && (
                  <div className="text-[11px] text-[var(--admin-text-secondary)]">
                    Project: <span className="text-[var(--gb-green-400)] font-semibold">{project.title}</span>
                  </div>
                )}
                <div className="text-[11px] text-[var(--admin-text-tertiary)] ml-auto">
        ...
        {filtered.length === 0 && (
          <div className="col-span-full">
            <Card variant="solid" className="p-10 text-center">
              <div className="text-4xl mb-3 opacity-50">NOTE</div>
              <p className="text-[var(--admin-text-secondary)]">No testimonials match your filters.</p>
            </Card>
          </div>
        )}
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <div className="bg-[var(--admin-surface-card)]/60 border border-[var(--admin-border)] rounded-xl px-4 py-3">
          ...
          <label className="flex items-center gap-3 p-3 rounded-xl bg-[var(--admin-surface-card)]/60 border border-[var(--admin-border)] cursor-pointer">
            <input
              type="checkbox"
              checked={form.featured}
              onChange={(e) => setForm({ ...form, featured: e.target.checked })}
              className="w-4 h-4 accent-[var(--gb-green-600)] cursor-pointer"
            />
            <span className="text-sm text-[var(--admin-text-primary)]">
              <span className="font-semibold">Pin as Featured</span>
              <span className="text-[var(--admin-text-secondary)]"> — Display on homepage carousel</span>
            </span>
          </label>
