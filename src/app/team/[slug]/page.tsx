...
import { usePersonBySlug } from '@/lib/api/hooks/usePeople';
...
export default function PersonProfilePage() {
  ...
  const { data: person, isLoading, isError, error, refetch } = usePersonBySlug(slug);

  if (isLoading) {
    return (
      <div className="min-h-screen pt-32 pb-20 flex items-center justify-center">
        <div className="animate-spin w-10 h-10 border-4 border-[color:var(--gb-green-2)] border-t-transparent rounded-full" />
      </div>
    );
  }
  ...
  if (!person) {
    return (
      <div className="min-h-screen pt-32 pb-20 text-center">
        <Container size="sm">
          <div className="rounded-3xl p-12 border bg-[color:var(--bg-soft)] border-[color:var(--border)]">
            <h1 className="text-2xl font-bold mb-2">Profile Not Found</h1>
            <p className="text-sm text-[color:var(--text-muted)] mb-6">
              We couldn&apos;t find a profile matching &quot;{slug}&quot;.
            </p>
            <Link href="/team">
              <Button variant="accent">← Back to Team Directory</Button>
            </Link>
          </div>
        </Container>
      </div>
