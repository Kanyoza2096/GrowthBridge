...
const SESSION_COOKIE = 'growthbridge-admin-session';
...
const ADMIN_DATA_API_PREFIX = '/api/admin/data/';
...
function getClientIp(request: NextRequest): string {
  const trustedEdgeIp = request.headers.get('cf-connecting-ip');
  if (trustedEdgeIp) return trustedEdgeIp.trim();
...
async function getSessionFromRequest(request: NextRequest): Promise<boolean> {
  try {
    ...
    if (!sessionCookie || sessionCookie.length < 50) return false;
    return true;
  } catch {
    return false;
  }
...
export async function middleware(request: NextRequest) {
  ...
  if (API_PATHS.some((p) => path.startsWith(p))) {
    ...
    if (path.startsWith(ADMIN_DATA_API_PREFIX) && !PUBLIC_PATHS.includes(path)) {
      const hasSession = await getSessionFromRequest(request);
      if (!hasSession) {
        return NextResponse.json(
    ...
    if (method !== 'GET' && method !== 'HEAD' && method !== 'OPTIONS' && path !== '/api/admin/login' && path !== '/api/admin/csrf-token') {
