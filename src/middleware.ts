...
/** Public form endpoints that receive stricter rate limits */
...
function getClientIp(request: NextRequest): string {
  // Prefer the hosting edge's canonical client IP. These headers must be
  // overwritten by the trusted reverse proxy in production.
...
function rateLimitResponse(result: Awaited<ReturnType<typeof checkApiRateLimit>>) {
...
export async function middleware(request: NextRequest) {
  ...
  if (API_PATHS.some((p) => path.startsWith(p))) {
    ...
    const maxBytes = path === '/api/admin/data/media' ? 10 * 1024 * 1024 : 256 * 1024;
    if (Number.isFinite(contentLength) && contentLength > maxBytes) {
      return NextResponse.json(
        { success: false, error: 'Request body is too large.' },
        { status: 413, headers: withHardenedHeaders() }
      );
    }
    ...
    // Stricter limit for public form endpoints (contact / apply / partner)
    const formKey = FORM_PATHS[path];
    if (formKey && method === 'POST') {
      const formRate = await checkFormRateLimit(ip, formKey);
      if (formRate.limited) {
        return rateLimitResponse(formRate);
      }
    }

    // Even stricter limit for admin login
    if (path === '/api/admin/login' && method === 'POST') {
      const loginRate = await checkLoginRateLimit(ip);
      if (loginRate.limited) {
        return rateLimitResponse(loginRate);
      }
    }

    // Protect admin data routes — require authenticated Supabase session
    ...
    // CSRF protection for all mutating admin API routes
    // (login and csrf-token endpoints are exempt)
    if (
      method !== 'GET' &&
      method !== 'HEAD' &&
      method !== 'OPTIONS' &&
    ...
    response.headers.set('X-RateLimit-Limit', String(apiPerMinute));
  ...
  // 4. Ensure CSRF cookie is present for the rest of the site
