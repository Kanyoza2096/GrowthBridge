...
/** Full person mapping — includes PII. Use only for authenticated admin paths. */
...
/**
 * Public-safe mapping — never includes email or phone.
 * Used for any path that may be reached by unauthenticated clients.
 */
function mapDbToPublicPerson(row: any): Person {
  return {
    id: row.id,
    slug: row.slug,
    category: row.category as PersonCategory,
...
export class PeopleRepository {
   ...
   * Always reads from the `public_people` view (no email/phone) and strips PII defensively.
   */
  async getPublicAll(params: PeopleQueryParams = {}): Promise<Person[]> {
    const supabase = await createServerClient();
    let query = supabase.from('public_people').select('*');
    ...
    if (error) {
      console.error('[PeopleRepository.getPublicAll]', error);
      throw error;
    }
    return (data || []).map(mapDbToPublicPerson);
  }

  /**
   * Public single person by slug.
   * Uses the `public_people` view so email/phone are never returned.
   */
  async getPublicBySlug(slug: string): Promise<Person | null> {
    const supabase = await createServerClient();
    ...
    if (!data) return null;
    return mapDbToPublicPerson(data);
  }

  /**
   * Admin / privileged listing (includes PII).
   * Prefer this only behind authenticated admin authorization.
   */
  ...
  async update(id: string, updates: Partial<Person>): Promise<Person> {
    ...
    if (updates.articles !== undefined) payload.articles = updates.articles;
